...
The FFI library provides no memory safety, unlike regular Lua code. It will happily allow you to dereference a `NULL` pointer, to access arrays out of bounds or to misdeclare C functions. If you make a mistake, the game might crash, just like equivalent C code would. It needs to be used with care, but it's flexibility and performance often outweigh this concern.
